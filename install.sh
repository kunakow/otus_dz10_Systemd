#!/bin/bash
sudo touch /etc/sysconfig/keyword
echo -e "WORD=\"ERROR\"\nLOG=/var/log/keyword.log" | sudo tee -a /etc/sysconfig/keyword
sudo touch /var/log/keyword.log
echo -e "ERROR" | sudo tee -a /var/log/keyword.log
sudo touch /opt/search.sh
echo -e " #!/bin/bash\nWORD=\$1\nLOG=\$2\nDATE=\`date\`\nif grep \$WORD \$LOG &> /dev/null\nthen\nlogger \"\$DATE: Bingo\"\nelse\nexit 0\nfi" | sudo tee -a /opt/search.sh
sudo chmod +x /opt/search.sh
sudo touch /etc/systemd/system/keyword.service
echo -e "[Unit]\nDescription=Find a keyword\n[Service]\nType=oneshot\nEnvironmentFile=/etc/sysconfig/keyword\nExecStart=/search.sh \$WORD \$LOG" | sudo tee -a /opt/search.sh
sudo touch /etc/systemd/system/keyword.timer
...
echo -e "PORT=8000\nPID_FILE=/etc/httpd/run/httpd1.pid" | sudo tee -a /etc/httpd/conf/httpd1.conf
echo -e "PORT=8001\nPID_FILE=/etc/httpd/run/httpd2.pid" | sudo tee -a /etc/httpd/conf/httpd2.conf
sudo setenforce 0
